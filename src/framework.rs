...
struct EventLoopWrapper<E: 'static> {
    event_loop: EventLoop<E>,
    window: Arc<Window>,
}

impl<E> EventLoopWrapper<E> {
...
impl SurfaceWrapper {
    ...
    fn start_condition<ExampleUserEvent>(e: &Event<ExampleUserEvent>) -> bool {
        match e {
    ...
    fn resume(&mut self, context: &ExampleContext, window: Arc<Window>, srgb: bool) {
        ...
        if srgb {
            ...
            let view_format = wgpu::TextureFormat::Rgba16Float;
...
async fn start<E: Example>(title: &str, 
    ...
    let mut example: Option<E> = None;

    cfg_if::cfg_if! {
        if #[cfg(target_arch = "wasm32")] {
            use winit::platform::web::EventLoopExtWebSys;
            let event_loop_function = EventLoop::<E::ExampleUserEvent>::spawn;
        } else {
            let event_loop_function = EventLoop::<E::ExampleUserEvent>::run;
        }
    }

    E::set_proxy(window_loop.event_loop.create_proxy());
    ...
    let _ = (event_loop_function)(
        ...
        move |event: Event<E::ExampleUserEvent>, target: &EventLoopWindowTarget<E::ExampleUserEvent>| {
            match event {
                winit::event::Event::UserEvent(event) => {
                    ...
                    example.as_mut().unwrap().handle_user_event(event);
                }
                ref e if SurfaceWrapper::start_condition::<E::ExampleUserEvent>(e) => {
...
#[cfg(target_arch = "wasm32")]
pub fn run<E: Example>(title: &'static str, logical_size: (u32, u32), container_name : String ) {
    wasm_bindgen_futures::spawn_local(async move { start::<E>(title, logical_size, container_name).await })
}
